...
const { TRONcrypto, sequelize } = require('../../db');

async function updateTronData() {
    try {
        const tableExists = await sequelize.getQueryInterface().showAllTables().then(tables => {
            return tables.includes('TRONcryptos');
        });

        if (tableExists) {
            await TRONcrypto.destroy({ where: {} });
        }
