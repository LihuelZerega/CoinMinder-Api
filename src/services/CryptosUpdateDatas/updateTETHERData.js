...
const { TETHERcrypto, sequelize } = require('../../db');

async function updateTetherData() {
    try {
        const tableExists = await sequelize.getQueryInterface().showAllTables().then(tables => {
            return tables.includes('TETHERcryptos');
        });

        if (tableExists) {
            await TETHERcrypto.destroy({ where: {} });
        }
        ...
        if (response.data && response.data.length > 0) {
            const tetherData = response.data[0];
...
module.exports = { updateTetherData };