...
async function updateBinancebitcoinData() {
    try {
        ...
        if (response.data && response.data.length > 0) {
            const binanncebitcoinData = response.data[0];
            ...
            console.log('Datos de Binancebitcoin actualizados correctamente.');
        } else {
            console.error('La respuesta de la API de CoinGecko está vacía o no contiene datos para Binancebitcoin.');
        }
    } catch (error) {
        console.error(`Error al actualizar los datos de Binancebitcoin: ${error.message}`);
    }
}
