...
async function fetchDataAndStoreInDatabase() {
  try {
    ...
    if (response.data && response.data.length > 0) {
      const lidostakedetherData = response.data[0];
      ...
      console.log('Datos de LidoStakedEther actualizados correctamente.');
    } else {
      console.error('La respuesta de la API de CoinGecko está vacía o no contiene datos para LidoStakedEther.');
    }
  } catch (error) {
    console.error('Error al actualizar los datos de LidoStakedEther:', error.message);
    throw error;
  }
}
