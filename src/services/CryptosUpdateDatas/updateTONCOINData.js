...
async function fetchDataAndStoreInDatabase() {
  try {
    ...
    if (response.data && response.data.length > 0) {
      const toncoinData = response.data[0];
      ...
      console.log('Datos de TONCOIN actualizados correctamente.');
    } else {
      console.error('La respuesta de la API de CoinGecko está vacía o no contiene datos para TONCOIN.');
    }
  } catch (error) {
    console.error('Error al actualizar los datos de TONCOIN:', error.message);
    throw error;
  }
}
